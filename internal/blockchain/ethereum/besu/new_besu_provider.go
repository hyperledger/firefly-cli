...
type NewBesuProvider struct {
	Log     log.Logger
	Verbose bool
	Stack   *types.Stack
}

func (p *NewBesuProvider) WriteConfig(options *types.InitOptions) error {
...
func (p *NewBesuProvider) FirstTimeSetup() error {
...
func (p *NewBesuProvider) PreStart() error {
	return nil
}

func (p *NewBesuProvider) PostStart() error {
	return nil
}

func (p *NewBesuProvider) DeployFireFlyContract() (*core.BlockchainConfig, error) {
	return ethconnect.DeployFireFlyContract(p.Stack, p.Log, p.Verbose)
}

func (p *NewBesuProvider) GetDockerServiceDefinitions() []*docker.ServiceDefinition {
	addresses := ""
	for i, member := range p.Stack.Members {
...
func (p *NewBesuProvider) GetFireflyConfig(stack *types.Stack, m *types.Member) (blockchainConfig *core.BlockchainConfig, orgConfig *core.OrgConfig) {
...
func (p *NewBesuProvider) getSmartContractAddressPatchJSON(contractAddress string) []byte {
	return []byte(fmt.Sprintf(`{"blockchain":{"ethereum":{"ethconnect":{"instance":"%s"}}}}`, contractAddress))
}

func (p *NewBesuProvider) Reset() error {
	return nil
}

func (p *NewBesuProvider) GetContracts(filename string, extraArgs []string) ([]string, error) {
...
func (p *NewBesuProvider) DeployContract(filename, contractName string, member *types.Member, extraArgs []string) (string, error) {
	return ethconnect.DeployCustomContract(member, filename, contractName)
}

func (p *NewBesuProvider) getEthconnectURL(member *types.Member) string {
	if !member.External {
